...
ccheck(){
	check=$(grep "$1" $TRAVIS_WORKING_DIR/build_rom.sh | wc -l)
...
init_check=$(grep 'repo init' $TRAVIS_WORKING_DIR/build_rom.sh | grep 'depth=1')
...
clone_check=$(grep 'git clone' $TRAVIS_WORKING_DIR/build_rom.sh | wc -l)
...
manifests_check=$(grep 'git clone' $TRAVIS_WORKING_DIR/build_rom.sh)
if [[ $manifests_check != *.repo/local_manifests* ]]; then echo Please follow git clone line from main branch.; exit 1; fi

url=https://$(grep 'repo init' $TRAVIS_WORKING_DIR/build_rom.sh -m 1 | cut -d / -f 3-5 | cut -d ' ' -f 1)
r_name=$(grep 'repo init' $TRAVIS_WORKING_DIR/build_rom.sh -m 1 | cut -d / -f 4)
...
j_check=$(tail $TRAVIS_WORKING_DIR/build_rom.sh -n +$(expr $(grep 'build/envsetup.sh' $TRAVIS_WORKING_DIR/build_rom.sh -n | cut -f1 -d:) - 1)| head -n -1 | grep -v 'rclone copy' | grep '\-j' | wc -l)
if [[ $j_check -gt 0 ]]; then echo Please dont specify j value in make line.; exit 1; fi

bliss_check=$(grep blissify $TRAVIS_WORKING_DIR/build_rom.sh | grep '\-c' | wc -l)
...
bliss_check=$(grep blissify $TRAVIS_WORKING_DIR/build_rom.sh | grep '\--clean' | wc -l)
if [[ $bliss_check -gt 0 ]]; then echo Please dont use make clean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

bliss_check=$(grep blissify $TRAVIS_WORKING_DIR/build_rom.sh | grep '\-d' | wc -l)
...
bliss_check=$(grep blissify $TRAVIS_WORKING_DIR/build_rom.sh | grep '\--devclean' | wc -l)
if [[ $bliss_check -gt 0 ]]; then echo Please dont use make installclean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

aospa_check=$(grep "rom-build.sh" $TRAVIS_WORKING_DIR/build_rom.sh | grep '\-c' | wc -l)
...
aospa_check=$(grep "rom-build.sh" $TRAVIS_WORKING_DIR/build_rom.sh | grep '\--clean' | wc -l)
if [[ $aospa_check -gt 0 ]]; then echo Please dont use make clean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

aospa_check=$(grep "rom-build.sh" $TRAVIS_WORKING_DIR/build_rom.sh | grep '\-i' | wc -l)
...
aospa_check=$(grep "rom-build.sh" $TRAVIS_WORKING_DIR/build_rom.sh | grep '\--installclean' | wc -l)
if [[ $aospa_check -gt 0 ]]; then echo Please dont use make installclean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

rclone_check=$(grep 'rclone copy' $TRAVIS_WORKING_DIR/build_rom.sh)
rclone_string="rclone copy out/target/product/\$(grep unch \$TRAVIS_WORKING_DIR/build_rom.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1)/*.zip travis:\$(grep unch \$TRAVIS_WORKING_DIR/build_rom.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1) -P"
if [[ $rclone_check != *$rclone_string* ]]; then echo Please follow rclone copy line of main branch.; exit 1; fi

sync_check=$(grep 'repo sync' $TRAVIS_WORKING_DIR/build_rom.sh)
...
rom_name=$(grep 'repo init' $TRAVIS_WORKING_DIR/build_rom.sh -m 1 | cut -d / -f 4)
branch_name=$(grep 'repo init' $TRAVIS_WORKING_DIR/build_rom.sh | awk -F "-b " '{print $2}' | awk '{print $1}')
...
device=$(grep unch $TRAVIS_WORKING_DIR/build_rom.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1)
grep _jasmine_sprout $TRAVIS_WORKING_DIR/build_rom.sh > /dev/null && device=jasmine_sprout
grep _laurel_sprout $TRAVIS_WORKING_DIR/build_rom.sh > /dev/null && device=laurel_sprout
grep _GM8_sprout $TRAVIS_WORKING_DIR/build_rom.sh > /dev/null && device=GM8_sprout
grep _maple_dsds $TRAVIS_WORKING_DIR/build_rom.sh > /dev/null && device=maple_dsds

if [[ $BRANCH != *pull/* ]]; then 
	if [[ $BRANCH != $device-$rom_name-* ]]; then echo Please use proper branch naming described in push group.; exit 1; fi; 
	if [[ $TRAVIS_COMMIT_MESSAGE == "Update build_rom.sh" ]]; then echo Please use proper commit message.; exit 1; fi; 
fi
...
if [[ $BRANCH == *pull/* ]]; then
	if [[ $TRAVIS_COMMIT_MESSAGE != $device-$rom_name-* ]]; then echo Please use proper PR label described in telegram group.; exit 1; fi
	lunch_check=$(grep "unch" $TRAVIS_WORKING_DIR/build_rom.sh | grep -v 'rclone' | wc -l)
	if [[ $rom_name != 'Corvus-R-12-test' ]]; then
		if [[ $lunch_check -gt 1 ]]; then echo Please build for one device at a time.; exit 1; fi
	fi
	cd /tmp/travis-ci-build
...
if [[ $TRAVIS_USER_PERMISSION == write ]]; then
	if [ -z "$TRAVIS_PR" ]; then true; else
		echo You are push user. Don\'t do pr and please follow pinned message in push group.; exit 1
	fi
fi

echo Test passed
echo "rom_name=$rom_name" >> $TRAVIS_ENV
echo "device=$device" >> $TRAVIS_ENV
