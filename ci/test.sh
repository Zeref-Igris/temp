...
ccheck(){
	check=$(grep "$1" $TRAVIS_BUILD_DIR/build_rom.sh | wc -l)
...
init_check=$(grep 'repo init' $TRAVIS_BUILD_DIR/build_rom.sh | grep 'depth=1')
...
clone_check=$(grep 'git clone' $TRAVIS_BUILD_DIR/build_rom.sh | wc -l)
...
manifests_check=$(grep 'git clone' $TRAVIS_BUILD_DIR/build_rom.sh)
if [[ $manifests_check != *.repo/local_manifests* ]]; then echo Please follow git clone line from main branch.; exit 1; fi

url=https://$(grep 'repo init' $TRAVIS_BUILD_DIR/build_rom.sh -m 1 | cut -d / -f 3-5 | cut -d ' ' -f 1)
r_name=$(grep 'repo init' $TRAVIS_BUILD_DIR/build_rom.sh -m 1 | cut -d / -f 4)
...
j_check=$(tail $TRAVIS_BUILD_DIR/build_rom.sh -n +$(expr $(grep 'build/envsetup.sh' $TRAVIS_BUILD_DIR/build_rom.sh -n | cut -f1 -d:) - 1)| head -n -1 | grep -v 'rclone copy' | grep '\-j' | wc -l)
if [[ $j_check -gt 0 ]]; then echo Please dont specify j value in make line.; exit 1; fi

bliss_check=$(grep blissify $TRAVIS_BUILD_DIR/build_rom.sh | grep '\-c' | wc -l)
...
bliss_check=$(grep blissify $TRAVIS_BUILD_DIR/build_rom.sh | grep '\--clean' | wc -l)
if [[ $bliss_check -gt 0 ]]; then echo Please dont use make clean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

bliss_check=$(grep blissify $TRAVIS_BUILD_DIR/build_rom.sh | grep '\-d' | wc -l)
...
bliss_check=$(grep blissify $TRAVIS_BUILD_DIR/build_rom.sh | grep '\--devclean' | wc -l)
if [[ $bliss_check -gt 0 ]]; then echo Please dont use make installclean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

aospa_check=$(grep "rom-build.sh" $TRAVIS_BUILD_DIR/build_rom.sh | grep '\-c' | wc -l)
...
aospa_check=$(grep "rom-build.sh" $TRAVIS_BUILD_DIR/build_rom.sh | grep '\--clean' | wc -l)
if [[ $aospa_check -gt 0 ]]; then echo Please dont use make clean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

aospa_check=$(grep "rom-build.sh" $TRAVIS_BUILD_DIR/build_rom.sh | grep '\-i' | wc -l)
...
aospa_check=$(grep "rom-build.sh" $TRAVIS_BUILD_DIR/build_rom.sh | grep '\--installclean' | wc -l)
if [[ $aospa_check -gt 0 ]]; then echo Please dont use make installclean flag. Server does make installclean by default, which is enough for most of the cases.; exit 1; fi

rclone_check=$(grep 'rclone copy' $TRAVIS_BUILD_DIR/build_rom.sh)
rclone_string="rclone copy out/target/product/\$(grep unch \$TRAVIS_BUILD_DIR/build_rom.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1)/*.zip travis:\$(grep unch \$TRAVIS_BUILD_DIR/build_rom.sh -m 1 | cut -d ' ' -f 2 | cut -d _ -f 2 | cut -d - -f 1) -P"
if [[ $rclone_check != *$rclone_string* ]]; then echo Please follow rclone copy line of main branch.; exit 1; fi

sync_check=$(grep 'repo sync' $TRAVIS_BUILD_DIR/build_rom.sh)
...
if [[ $BRANCH == *pull/* ]]; then
	...
	lunch_check=$(grep "unch" $TRAVIS_BUILD_DIR/build_rom.sh | grep -v 'rclone' | wc -l)
